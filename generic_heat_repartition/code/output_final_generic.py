...
os.chdir('C:/MYDIRECTORY/generic_heat_repartition')

#### PUT YOUR NAICS CODE HERE
naics_code = 000000

#### PUT YOUR PROCESS NAME HERE, YOU SHOULD TRY TO ADD IN A 'plant type' COLUMN THAT YOU HAVE CURATED IN THE DATASET THAT ALLOWS YOU TO IDENTIFY THE PROCESS
#### IF YOU HAVEN'T/CAN'T, UNCOMMENT LINE 45 AND DO IT THERE AND ANY PLANT IN THE NAICS CODE WILL BE THAT PROCESS

myprocess_name = 'myprocess'



#Load process parameters IMPORT YOUR PROCESS PARAMETERS HERE!!!
myprocess_units = pd.read_excel('data/MYPROCESS.xlsx', sheet_name= 'unit heat demand')
myprocess_combustion_units = pd.read_excel('data/MYPROCESS.xlsx', sheet_name= 'unit types')
myprocess_non_electrifiable_fuels = pd.read_excel('data/MYPROCESS.xlsx', sheet_name= 'non electrifiable fuels')


#Load data
unit_emissions = pd.read_excel('data/Facility_and_Unit_Emissions_Database_2023_v3.xlsx', sheet_name= 'unit')

facility_info = pd.read_excel('data/Facility_and_Unit_Emissions_Database_2023_v3.xlsx', sheet_name= 'facility')
...
unit_emissions2 = unit_emissions2[unit_emissions2['primary_naics'] == naics_code]



#unit_emissions2['plant type'] = myprocess_name
...
#############################ONLY RELEVANT TO PULP AND PAPER######################
...
#Need to do some different stuff for AA subparts ONLY RELEVANT FOR PULP AND PAPER
...
combustion_unit_correspondence = pd.merge(unit_info, myprocess_combustion_units[['unit_type','combustion_unit_category', 'process_unit' ,'combustion_unit_electrifiable']], how = 'left')




#Limit to only given processes

myprocess_filtered_df = combustion_unit_correspondence[combustion_unit_correspondence['plant type'] == myprocess_name]



#Do a iteration process to treat each facility

facility_list = myprocess_filtered_df['facility_id'].unique().tolist()
process_unit_list = myprocess_units['Unit name'].tolist()
...
for facility in facility_list:
    plantspecific_df = myprocess_filtered_df[myprocess_filtered_df['facility_id'] == facility]
    process_units_copy = myprocess_units.copy()
    comb_units_copy = myprocess_combustion_units.copy()
    elec_fuel_copy = myprocess_non_electrifiable_fuels.copy()
