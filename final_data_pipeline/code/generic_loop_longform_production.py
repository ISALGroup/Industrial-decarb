...
for naics in generic_naics_list:
    ...
    process_units = pd.read_excel('data/parameter_sheets/' + str(naics) + '_process_parameters.xlsx', sheet_name= 'unit heat demand')
    combustion_units = pd.read_excel('data/parameter_sheets/' + str(naics) + '_process_parameters.xlsx', sheet_name= 'unit types')
    non_electrifiable_fuels = pd.read_excel('data/parameter_sheets/' + str(naics) + '_process_parameters.xlsx', sheet_name= 'non electrifiable fuels')
    ...
    unit_emissions2['plant type'] = naics
    ...
    result_c = c_df.groupby(
        ['unit_type', 'unit_name', 'subpart', 'fuel_type','is_biogenic', 'primary_naics','facility_id',	'reporting_year',
         'facility_name', 'state', 'naics_title', 'plant type', 'is_redundant'
    ...
    combustion_unit_correspondence = pd.merge(unit_info, combustion_units[['unit_type','combustion_unit_category', 'process_unit' ,'combustion_unit_electrifiable']], how = 'left')
    ...
    kraft_filtered_df = combustion_unit_correspondence[combustion_unit_correspondence['plant type'] == naics]
    ...
    process_unit_list = process_units['Unit name'].tolist()
    ...
    for facility in facility_list:
        ...
        process_units_copy = process_units.copy()
        comb_units_copy = combustion_units.copy()
        elec_fuel_copy = non_electrifiable_fuels.copy()
    ...
    pivot_columns = ['unit_type', 'unit_name', 'subpart', 'fuel_type', 'is_biogenic', 'primary_naics', 'facility_id', 'reporting_year', 'facility_name', 'state', 'naics_title', 'plant type', 'is_redundant', 'ghg_quantity', 'emission_factor', 'fuel_hhv_mmbtu', 'thermal_eff' , 'elec_eff', 'process_heat', 'combustion_unit_category', 'process_unit', 'combustion_unit_electrifiable']
    ...
    added_process_temps = pd.merge(sorted_longform_spread_heat, process_units[['Unit name','Energy demand','Temperature','Equivalent unit type in GHGRP',	'Direct electrification', 'Utility electrifiable']], left_on='individual_process_unit', right_on= 'Unit name', how= 'left')
...
#    added_process_temps['average_county_temperature'] = 10
    ...
    non_elec_fuel_list = non_electrifiable_fuels['fuels list'].unique().tolist()
    ...
    electrified_option_df['electrified_option'] = electrified_option_df.apply(electrification_option, args =(process_units,), axis = 1)
    
    electrified_option_df.to_excel('output/' + str(naics)+  '_elec_options.xlsx')
    ...
    waste_heat_pump_df['whp_option'] = waste_heat_pump_df.apply(whp_decision, args =(process_units,), axis = 1)
    
    waste_heat_pump_df['whp_inlet_t'] = waste_heat_pump_df.apply(whp_temp, args =(process_units,), axis = 1)
    ...
    electrification_df.to_excel('output/' + str(naics) + 'longform.xlsx')
    ...
    sorted_elec_df.to_excel('output/' + str(naics) + 'longform_elec_options.xlsx')
    ...
    no_whp_df.to_excel('output/' + str(naics) + 'longform_elec_options_nowhp.xlsx')
