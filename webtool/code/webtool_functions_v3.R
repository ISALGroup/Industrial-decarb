...
# Version Notes
## Adds PTC 
## Adds yearly O&M costs as a variable, so it does not have to be calculated in the function

## Search NEW to see where things changed 
...
webtool_df <- read_csv('webtool/data/webtool_data_20251104.csv')
...
lcoh_func <- function(
  ...
  yearly_opm_costs,                        # NEW: Yearly operations and maintenance costs 
  ...
  ## NEW: We are removing these in favor of just supplying you the O&M for each scenario 
  ...
  case_when(
    ...
    # Electrification LCOH 
    tech_scenario != 'baseline' ~ {
      capex_adj <- capex * (1 - capex_subsidy)
      opex_elec <- change_in_electricity_demand_kwh * (elec_price * (1 - elec_discount))
      ptc_credit <- heat_mmbtu * PTC
      
      numerator   <- capex_adj + ((yearly_opm_costs + opex_elec - ptc_credit) * discount_sum)
