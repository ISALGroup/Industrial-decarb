...
onsite_facility_emissions <- unit_emissions_df %>% 
  ...
  group_by(facility_id, is_biogenic, electrified_option) %>% 
  summarize(
...
onsite_facility_nonelec <- onsite_facility_emissions %>% 
...
baseline_facility_emission <- facility_level_emissions_df %>% 
  ...
  select(facility_id,scenario_rank, baseline_co2e_emissions = base_emissions, policy_year)
...
bau_grid_combined_total_emissions <- bau_grid_facility_emissions %>% 
  left_join(onsite_facility_nonelec, by="facility_id") %>% 
...
bau_grid_simple <- bau_grid_combined_total_emissions %>% 
  filter(policy_year == 2025,
         scenario_rank == 'best') %>% 
  select(-reporting_year, -county_fips, -subregion, -naics_title, -sector, -scenario_rank, -current_clean_share, -policy_year, 
         -bau_co2e_kg_kwh, -bau_so2_kg_kwh, -bau_nox_kg_kwh, -bau_pm25_kg_kwh)
