...
# 8/15: updated lcoh formula, added fuel_type
...
tech_input_df <- read_excel("LCOH modelling/output/longform_mn_updatedfuelslist.xlsx") %>%
  mutate(facility_name = tolower(facility_name)) %>% 
  mutate(baseline_co2e_emissions = elec_ghg_emissions + noelec_ghg_emissions) %>% 
  rename(fuel_type = fuels) %>% 
  select(-1, -facility_id, -opex) #removing opex bc will calculate in this code
...
tech_combined_df <- tech_input_df %>%
  ...
  select(facility_id, facility_name, state, county_fips, latitude, longitude, 
         subregion, sector, naics_code, naics_description, tech_scenario, change_in_electricity_demand_kwh, 
         heat_mmbtu, capex, fuel_type, baseline_co2e_emissions,elec_ghg_emissions, noelec_ghg_emissions,
         current_fossil_share, current_clean_share, co2e_kg_kwh, nox_kg_kwh, so2_kg_kwh, pm25_kg_kwh)
...
facility_emissions_long_df <- rbind(facility_emissions_baseline, facility_emissions_no_baseline) %>% 
  ...
  select(facility_id, facility_name, state, county_fips, latitude, longitude, naics_code, naics_description, 
         sector, subregion, tech_scenario, heat_mmbtu, fuel_type, change_in_electricity_demand_kwh, 
...
# 8/15 this should be replaced by 2 parameters files in the future, one for the technology scenarios and one for costs/baseline lcoh by state
r <- 0.065
t <- 15
elec_price <- 0.092
labor <- 0.02 # assuming labor cost is 2 % of capex
discount_sum <- sum((1 + r)^(0:(t - 1)))

# --- Calculate LCOH with Policy Scenarios ---
# 8/15: this calculates LCOH for baseline scenario but that should be overwritten with state-level LCOH input file (to come later)
facility_lcoh_df <- tidyr::crossing(tech_combined_df, policy_grid) %>%
  mutate(
    ...
    opex_elec = change_in_electricity_demand_kwh * elec_price_adj,
    opex_labor = labor * capex, 
    lcoh_adj = ((capex / discount_sum) + (opex_labor / discount_sum) + (opex_elec / discount_sum)) / (heat_mmbtu/discount_sum),
    policy_label = case_when(
      capex_subsidy == 0 & elec_discount == 0 ~ paste0("No Policy"),
      TRUE ~ paste0("Capex: -", capex_subsidy * 100, "%, Elec: -", elec_discount * 100, "%")
    )
  ) %>% 
  select(facility_id, facility_name, state, county_fips, latitude, longitude, naics_code, naics_description, sector, 
         subregion, tech_scenario, heat_mmbtu, fuel_type, change_in_electricity_demand_kwh, 
         policy_label, capex_subsidy, capex_adj, elec_discount, elec_price_adj, opex_elec, opex_labor, lcoh_adj 
         )

# LCOH summary 
state_lcoh_summary <- facility_lcoh_df %>% 
  ...
  summarise(
    ...
    weighted_LCOH    = sum(heat_mmbtu * lcoh_adj, na.rm = TRUE) / total_heat_mmbtu,
    .groups = "drop"
  )

county_lcoh_summary <- facility_lcoh_df %>% 
...
#### DATA EXPORT ####
# facility level
writexl::write_xlsx(facility_lcoh_df, "state_fact_sheets/data/modified/state-data/MN/250815_facility_lcoh_results_mn_v2.xlsx") 
writexl::write_xlsx(facility_emissions_long_df, "state_fact_sheets/data/modified/state-data/MN/250815_facility_emissions_results_mn_v2.xlsx") 

# county level
writexl::write_xlsx(county_emissions_summary, "state_fact_sheets/data/modified/state-data/MN/250815_county_emissions_results_mn_v4.xlsx")
writexl::write_xlsx(county_lcoh_summary, "state_fact_sheets/data/modified/state-data/MN/250815_county_lcoh_results_mn_v4.xlsx")

# state level
writexl::write_xlsx(state_emissions_summary, "state_fact_sheets/data/modified/state-data/MN/250815_state_emissions_results_mn_v4.xlsx")
writexl::write_xlsx(state_lcoh_summary, "state_fact_sheets/data/modified/state-data/MN/250815_state_lcoh_results_mn_v4.xlsx")

# webtool input example data 
writexl::write_xlsx(tech_combined_df, "state_fact_sheets/data/modified/state-data/MN/250811_facility_option_b_sample_mn_v4.xlsx")
