...
rm(facility_info, natgas_best, tech_input_df, tech_input_df.o)
...
lcoh_tech_sector <- 
  ...
  filter(!is.na(industry_clean)) # drop NA sectors
...
# Cumulative emissions first 
# ---- User inputs for decarbonization trajectory (used for cumulative series) ----
...
# Helper: interpolate named target vector to full years
...
# Build targets table per subregion (apply global targets to all subregions)
...
grid_yearly_scaled <- egrid_df %>%
  ...
  mutate(
    ...
    scaled_nox_kg_kwh_year  = nox_kg_kwh  * scaling_factor,
    scaled_so2_kg_kwh_year  = so2_kg_kwh  * scaling_factor,
    scaled_pm25_kg_kwh_year = pm25_kg_kwh * scaling_factor
  ) %>%
  select(subregion, year, current_fossil_share, current_clean_share, target_clean, scaling_factor,
         scaled_co2e_kg_kwh_year, scaled_nox_kg_kwh_year, scaled_so2_kg_kwh_year, scaled_pm25_kg_kwh_year)

# ---- Facility × year table (annual CO2e) ----
facility_yearly <- tech_combined_df %>%
  ...
  select(-match_score, -match_type, -fuel_reduction, -temperature)
...
facility_yearly_avg <- facility_yearly %>%
  ...
  group_by(facility_id, facility_name, state, subregion, sector, naics_code, naics_description, 
           industry_clean, tech_scenario, year) %>%
  summarise(
    annual_emissions_avg_tonnes = mean(annual_emissions_co2e_tonnes, na.rm = TRUE),
    .groups = "drop"
  )

# ---- Cumulative CO2e (2030-2050) by sector & tech_scenario (100% clean trajectory) ----
cumulative_co2e_by_scenario_sector <- facility_yearly_avg %>%
  filter(year >= 2030, year <= 2050) %>%
  filter(!is.na(industry_clean)) %>%
  group_by(sector, tech_scenario, naics_description, industry_clean) %>%
  summarise(cum_tco2e = sum(annual_emissions_avg_tonnes, na.rm = TRUE), .groups = "drop") %>%
  mutate(cum_MtCO2e = cum_tco2e / 1e6, grid_scenario = "Projected Clean Grid")

# ---- BAU cumulative (use 2025 intensity for all years 2030-2050) ----
bau_grid_2025 <- grid_yearly_scaled %>%
  filter(year == 2025) %>%
  select(subregion, bau_co2e_kg_kwh_2025 = scaled_co2e_kg_kwh_year)
...
facility_yearly_bau <- tech_combined_df %>%
  ...
  mutate(tech_scenario = str_remove(tolower(tech_scenario), regex("(best|worst)$", ignore_case = TRUE))) %>%
  group_by(facility_id, facility_name, state, subregion, sector, naics_code, naics_description, industry_clean,
           tech_scenario, year) %>%
  summarise(annual_emissions_avg_tonnes_bau = mean(annual_emissions_co2e_tonnes_bau, na.rm = TRUE), .groups = "drop")

bau_cum_co2e <- facility_yearly_bau %>%
  group_by(sector, tech_scenario, naics_description) %>%
  summarise(cum_tco2e = sum(annual_emissions_avg_tonnes_bau, na.rm = TRUE), .groups = "drop") %>%
  mutate(cum_MtCO2e = cum_tco2e / 1e6, grid_scenario = "BAU")

# ---- Combine cumulative series for plotting ----
plot_df <- bind_rows(bau_cum_co2e, cumulative_co2e_by_scenario_sector) 

# ---- Present-day snapshots (2025 intensity = "Current", plus 80% and 100% cleaner computed consistently) ----
# Build snapshots per subregion: target_clean = current_clean_share, 0.8, 1.0 (capped / no degradation)
snapshot_targets_tbl <- egrid_df %>%
  left_join(grid_mix_df, by = "subregion") %>%
  rowwise() %>%
  mutate(
    tmp = list(tibble(target_clean = pmin(1, pmax(0, c(current_clean_share, 0.8, 1.0)))))
  ) %>%
  tidyr::unnest(cols = c(tmp)) %>%
  ungroup() %>%
  mutate(
    target_fossil_share = 1 - target_clean,
    scaling_factor = if_else(current_fossil_share > 0, target_fossil_share / current_fossil_share, 0),
    co2e_kg_kwh_snapshot = co2e_kg_kwh * scaling_factor,
    nox_kg_kwh_snapshot  = nox_kg_kwh  * scaling_factor,
    so2_kg_kwh_snapshot  = so2_kg_kwh  * scaling_factor,
    pm25_kg_kwh_snapshot = pm25_kg_kwh * scaling_factor,
    clean_grid_scenario_label = if_else(target_clean == current_clean_share, "Current Grid Mix", paste0(round(target_clean * 100), "% Clean Grid"))
  ) %>%
  select(subregion, clean_grid_scenario_label, co2e_kg_kwh_snapshot, nox_kg_kwh_snapshot, so2_kg_kwh_snapshot, pm25_kg_kwh_snapshot)

# Join snapshots to facilities and compute "present" totals (2025-based)
facility_present_long <- tech_combined_df %>%
  left_join(snapshot_targets_tbl, by = "subregion") %>%
  # pivot intensities into pollutant_type so we can use same mapping logic as before
  tidyr::pivot_longer(
    cols = ends_with("_kg_kwh_snapshot"),
    names_to = "pollutant_type",
    names_pattern = "^(.*)_kg_kwh_snapshot$",
    values_to = "grid_emissions_kg_kwh"
  ) %>%
  mutate(
    # make one emissions variable, calibrated to copollutant & scenario (mimics your colleague's earlier mapping)
    ...
    total_emissions = if_else(
      str_detect(tech_scenario, "Baseline"),
      facility_emissions,
      (change_in_electricity_demand_kwh * grid_emissions_kg_kwh) / 1000 + facility_emissions
    )
  ) %>%
  select(-capex, -heat_mmbtu, -elec_ghg_emissions, -noelec_ghg_emissions, -nox_emissions, -so2_emissions, -pm25_emissions)

state_emissions_snapshot <- facility_present_long %>%
  filter(!is.na(industry_clean)) %>%
  group_by(industry_clean, clean_grid_scenario_label, tech_scenario, pollutant_type) %>%
  summarise(total_emissions = sum(total_emissions, na.rm = TRUE), .groups = "drop") %>%
  mutate(emissions_Mt = total_emissions / 1e6)

# Prepare the present-day plotting dataset
clean_grid_levels <- c("Current Grid Mix",  "Projected Clean Grid")

emissions_df <- state_emissions_snapshot %>%
  filter(clean_grid_scenario_label %in% clean_grid_levels) %>%
  mutate(
    scenario_base = str_remove(tech_scenario, regex("Best|Worst", ignore_case = TRUE)),
    clean_grid_scenario_label = factor(clean_grid_scenario_label, levels = clean_grid_levels)
  ) %>%
  filter(str_detect(tech_scenario, 'Best')) %>% # keep 'Best' case as previously used
  group_by(industry_clean, clean_grid_scenario_label, scenario_base, pollutant_type) %>%
  summarise(total_emissions = sum(total_emissions, na.rm = TRUE), .groups = "drop") %>%
  mutate(emissions_Mt = total_emissions / 1e6)

# ---- Present-day plots (now computed from same intensity logic as cumulative plots) ----
co2e_plot <- emissions_df %>%
  filter(pollutant_type == 'co2e') %>%
  ggplot2::ggplot(aes(x = industry_clean, y = emissions_Mt, fill = scenario_base)) +
  ggplot2::geom_col(position = ggplot2::position_dodge(width = 0.8), width = 0.6) +
  ggplot2::facet_wrap(~ clean_grid_scenario_label, nrow = 1) +
  ggplot2::labs(x = NULL, y = "GHG Emissions (Mt CO2e)", fill = "Tech Scenario") +
  ggplot2::scale_fill_manual(values = scenario_colors, labels = scenario_labels) +
  ggplot2::scale_y_continuous(limits = c(0, max(emissions_df$emissions_Mt, na.rm = TRUE) * 1.1), expand = c(0, 0)) +
  ggplot2::theme_bw(base_size = 14) +
  ggplot2::theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 9),
                 legend.text = element_text(size = 10),
                 legend.title = element_text(size = 11),
                 legend.background = element_rect(color = "black", linewidth = 0.2))
co2e_plot

so2_plot <- emissions_df %>%
  filter(pollutant_type == 'so2') %>%
  ggplot2::ggplot(aes(x = industry_clean, y = total_emissions, fill = scenario_base)) +
  ggplot2::geom_col(position = ggplot2::position_dodge(width = 0.8), width = 0.6) +
  ggplot2::facet_wrap(~ clean_grid_scenario_label, nrow = 1) +
  ggplot2::labs(x = NULL, y = expression("SO"[2]*" Emissions (t)"), fill = "Tech Scenario") +
  ggplot2::scale_fill_manual(values = scenario_colors, labels = scenario_labels) +
  ggplot2::theme_bw(base_size = 14) +
  ggplot2::theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 9),
                 legend.text = element_text(size = 10),
...
nox_plot <- emissions_df %>%
  filter(pollutant_type == 'nox') %>%
  ggplot2::ggplot(aes(x = industry_clean, y = total_emissions, fill = scenario_base)) +
  ggplot2::geom_col(position = ggplot2::position_dodge(width = 0.8), width = 0.6) +
  ggplot2::facet_wrap(~ clean_grid_scenario_label, nrow = 1) +
  ggplot2::labs(x = NULL, y = expression("NO"[x]*" Emissions (t)"), fill = "Tech Scenario") +
  ggplot2::scale_fill_manual(values = scenario_colors, labels = scenario_labels) +
  ggplot2::theme_bw(base_size = 14) +
  ggplot2::theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 9),
                 legend.text = element_text(size = 10),
...
pm25_plot <- emissions_df %>%
  filter(pollutant_type == 'pm25') %>%
  ggplot2::ggplot(aes(x = industry_clean, y = total_emissions, fill = scenario_base)) +
  ggplot2::geom_col(position = ggplot2::position_dodge(width = 0.8), width = 0.6) +
  ggplot2::facet_wrap(~ clean_grid_scenario_label, nrow = 1) +
  ggplot2::labs(x = NULL, y = expression("PM"[2.5]*" Emissions (t)"), fill = "Tech Scenario") +
  ggplot2::scale_fill_manual(values = scenario_colors, labels = scenario_labels) +
  ggplot2::theme_bw(base_size = 14) +
  ggplot2::theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 9),
                 legend.text = element_text(size = 10),
...
# ---- Cumulative plots (your code / unchanged plotting style) ----
# set up plotting palettes & ordering (lowercased tech_scenario keys)
scenario_fill <- c(
  "baseline" = "#fb9a99",
  "scenario1" = "#1f78b4",
  "scenario3" = "#a6cee3",
...
naics_order <- c("Pulp & Paper", "Ethanol", "Beet Sugar", "Spices","Soybeans")

plot_df <- plot_df %>%
  mutate(
    naics_description = factor(naics_description, levels = naics_order),
    sector = factor(sector, levels = sector_order),
    tech_scenario = factor(tolower(tech_scenario), levels = names(scenario_fill))
  )
...
p_bau <- plot_df %>%
  ...
  ggplot2::ggplot(aes(x = naics_description, y = cum_MtCO2e, fill = tech_scenario)) +
  ggplot2::geom_col(position = ggplot2::position_dodge(width = 0.8), width = 0.7) +
  ggplot2::labs(title = "Business-As-Usual Grid Scenario", x = NULL, y = "Cumulative MMt CO2e (2030-2050)", fill = "Tech scenario") +
...
p_clean <- plot_df %>%
  filter(grid_scenario == "Projected Clean Grid") %>%
  ggplot2::ggplot(aes(x = naics_description, y = cum_MtCO2e, fill = tech_scenario)) +
  ggplot2::geom_col(position = ggplot2::position_dodge(width = 0.8), width = 0.7) +
  ggplot2::labs(title = "Clean Grid Projected Scenario", x = NULL, y = NULL, fill = "Tech scenario") +
  ggplot2::scale_fill_manual(values = scenario_fill, labels = scenario_labels) +
  ggplot2::scale_y_continuous(limits = c(0, y_max)) +
  ggplot2::theme_minimal(base_size = 12) +
  ggplot2::theme(legend.position = "bottom")

combined_plot <- p_bau + p_clean + patchwork::plot_layout(guides = "collect") & ggplot2::theme(legend.position = "bottom")
...
ggsave(glue("state_fact_sheets/outputs/state-fact-sheet-figures/{st}/{st}_lcoh_v_elec_plot_{format(Sys.Date(), '%Y%m%d')}.png"),
       lcoh_v_elec_plot, 
       width = 8, height = 5, dpi = 300)

ggsave(glue("state_fact_sheets/outputs/state-fact-sheet-figures/{st}/{st}_eim_plot_{format(Sys.Date(), '%Y%m%d')}.png"),
       eim_plot, 
       width = 8, height = 5, dpi = 300)
