...
plot_df <- bind_rows(bau_cum_co2e, cumulative_co2e_by_scenario_sector) %>% 
  mutate(naics_description = case_when(
    naics_description == "Ethyl Alcohol Manufacturing" ~ "Ethanol",
    ...
    naics_description %in% c("Paper Mills", "Paperboard Mills", "Pulp Mills") ~ "Pulp & Paper",
...
print(combined_plot)
ggsave("state_fact_sheets/outputs/state-fact-sheet-figures/MI/mi_cumulative_emissions_250929.png", width = 8, height = 5, dpi = 300)
