...
state_emissions_summary <-
  facility_emissions_long_df %>%
  group_by(state, sector, naics_code, naics_description, tech_scenario, clean_grid_scenario_label, pollutant_type) %>%
  summarise(
...
prices <- 
  ...
  select(state, elec_price, ng_price)
...
#### DATA EXPORT ####
writexl::write_xlsx(policy_applied_df, glue("state_fact_sheets/data/modified/state-data/Combined/combined_lcoh_{format(Sys.Date(), '%Y%m%d')}.xlsx"))

writexl::write_xlsx(state_emissions_summary, glue("state_fact_sheets/data/modified/state-data/Combined/combined_emissions_{format(Sys.Date(), '%Y%m%d')}.xlsx"))
