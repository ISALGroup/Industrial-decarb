...
delta_bubble_data <- 
  ...
  select(name, label, sector, lcoh_delta, base_emissions) 
...
size_lims <-
  range(c(delta_bubble_data_fnb %N>% pull(base_emissions),
          delta_bubble_data_pnp %N>% pull(base_emissions),
          delta_bubble_data_chem %N>% pull(base_emissions)
  ), 
  na.rm = TRUE)
...
global_min <- 
  min(c(
      delta_bubble_data_fnb %N>% pull(base_emissions),
      delta_bubble_data_pnp %N>% pull(base_emissions),
      delta_bubble_data_chem %N>% pull(base_emissions)
    ), na.rm = TRUE)
    ...
    geom_node_text(
      data = function(d) d %>%
        mutate(
          ...
          label_size = rescale(sqrt(pmax(base_emissions, 0)),
...
payback_data_nopol <- 
  ...
  #   top15 <- . |>
  #     group_by(state_sub) |>
  #     summarize(mean_payback = mean(payback_years, na.rm = TRUE)) |>
  #     slice_min(mean_payback, n = 15)
  #   
  #   semi_join(., top15, by = "state_sub")
  # }

payback_plot_nopol <- 
  ggplot(payback_data_nopol,
         aes(x = reorder(state_sub, payback_years),
             y = payback_years,
             fill = sector)) +
