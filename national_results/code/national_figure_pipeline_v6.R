...
payback_bubble_data <- 
  ...
  select(state, statesub, statesub_label, industry_clean, sector, payback_years, elec_mt_25) 
...
hier_data <- 
  ...
  summarize(
    state = first(state),
    elec_mt_25 = sum(elec_mt_25, na.rm = TRUE),
...
#### FIG: NATIONAL EMISSIONS "IN THE MONEY" (OLD) ####
fig_policies <- c(
  "Baseline In-Scope Emissions", 
  "No Policy",
  "ITC Capex -100%", 
  "Elec -25%", 
  "Elec -50%", 
...
#### FIG: STATE EMISSIONS "IN THE MONEY" (OLD) ####
fig_policies <- c(
