...
facility_lcoh_df <- 
  read_csv('/Users/nmariano/Library/CloudStorage/Dropbox/1 Projects/national-results-data/lcoh_industrialdecarb_facility_level.csv') |>
  # Name & value clean-up 
  rename(
...
# Also, we need to drop the figures 
emissions_df <- 
  ...
  summarize(
    ...
    elec_ghg_emissions = sum(if_else(combustion_unit_electrifiable == 'Y' & !is_biogenic, unit_ghg_emissions, 0), na.rm = TRUE),
    biogenic_ghg_emissions = sum(if_else(is_biogenic, unit_ghg_emissions, 0), na.rm = TRUE),
    noelec_ghg_emissions = sum(if_else(combustion_unit_electrifiable == 'N', unit_ghg_emissions, 0), na.rm = TRUE),
  ) |>
  mutate(
    facility_id = as.character(facility_id)
  ) %>%
...
capex_data <- 
  ...
  mutate(
    ...
    heat_kw = elec_orig_process_unit_heat_demand * 293.071 / 8000,
...
ggsave(glue("national_results/outputs/capex_plot_{format(Sys.Date(), '%Y%m%d')}.png"),
       capex_plot,
...
ggsave(glue("national_results/outputs/delta_lcoh_plot_{format(Sys.Date(), '%Y%m%d')}.png"),
       delta_lcoh_plot,
       width = 8, height = 5, dpi = 300)
...
# --- normalize base_emissions globally so circle sizes are comparable ---
global_min <- min(c(
  delta_bubble_data_good %N>% pull(base_emissions),
  delta_bubble_data_bad  %N>% pull(base_emissions)
), na.rm = TRUE)
global_max <- max(c(
  delta_bubble_data_good %N>% pull(base_emissions),
  delta_bubble_data_bad  %N>% pull(base_emissions)
...
delta_bubble_data_good <- delta_bubble_data_good %N>% 
  mutate(weight_scaled = rescale_emissions(base_emissions))
delta_bubble_data_bad  <- delta_bubble_data_bad  %N>% 
  mutate(weight_scaled = rescale_emissions(base_emissions))
...
delta_bubble_plot <- 
  (base_plot(delta_bubble_data_good, "Top 15 'Hot Spots'") + 
   base_plot(delta_bubble_data_bad, "Top 15 'Challenge Spots'")
   ) + 
  plot_layout(guides = "collect") &
  theme(legend.position = "right")

ggsave(glue("national_results/outputs/delta_bubble_plot_{format(Sys.Date(), '%Y%m%d')}.png"),
       delta_bubble_plot,
       width = 8, height = 5, dpi = 300)
...
delta_bubble_wgt_data_good <- 
  ...
  summarize(
    ...
    # WEIGHTED lcoh delta
    lcoh_delta = weighted.mean(lcoh_delta, base_emissions, na.rm = T), 
    base_emissions = sum(base_emissions, na.rm = T)
  ) |>
  ungroup() |>
  mutate(name = paste(state, industry_clean, sep = " "), 
         label = str_wrap(name, width = 10)) |>
  select(state, industry_clean, name, label, lcoh_delta, base_emissions) |>
  slice_min(order_by = lcoh_delta, n = 15) |>
  tbl_graph(nodes = _, edges = NULL) 
...
size_lims <- range(c(delta_bubble_wgt_data_good %N>% pull(base_emissions),
                     delta_bubble_wgt_data_bad %N>% pull(base_emissions)), na.rm = TRUE)
...
# --- normalize base_emissions globally so circle sizes are comparable ---
global_min <- min(c(
  delta_bubble_wgt_data_good %N>% pull(base_emissions),
  delta_bubble_wgt_data_bad  %N>% pull(base_emissions)
), na.rm = TRUE)
global_max <- max(c(
  delta_bubble_wgt_data_good %N>% pull(base_emissions),
  delta_bubble_wgt_data_bad  %N>% pull(base_emissions)
...
ggsave(glue("national_results/outputs/delta_policy_plot_{format(Sys.Date(), '%Y%m%d')}.png"),
       delta_policy_plot,
       width = 8, height = 5, dpi = 300)
...
total_elec_ghg_emissions <- 
  ...
  summarize(
    ...
    policy_label = "Total Electrifiable Emissions"
  ) %>%
...
eim_plot <- 
  ...
  scale_fill_manual(
    values = c(
      "Total Electrifiable Emissions" = "grey70",
      "No Policy"                     = "#1b9e77",
      "Capex -100%"                  = "#d95f02",
      "Elec -25%"                    = "#7570b3",
      "Elec -50%"                    = "lightblue"
...
ggsave(glue("national_results/outputs/eim_plot_{format(Sys.Date(), '%Y%m%d')}.png"),
       eim_plot,
       width = 8, height = 5, dpi = 300)
  ...
  bind_rows(
    facility_lcoh_df %>%
      ...
      summarise(across(where(is.numeric), mean, na.rm = TRUE),
                across(c(facility_name, state, industry_clean, tech_scenario, policy_label), first),
                .groups = "drop") %>%
...
ggsave(glue("national_results/outputs/acc_plot_{format(Sys.Date(), '%Y%m%d')}.png"),
       acc_plot,
...
ggsave(glue("national_results/outputs/sg_ratio_plot_{format(Sys.Date(), '%Y%m%d')}.png"),
       delta_opex_plot,
       width = 8, height = 5, dpi = 300)
