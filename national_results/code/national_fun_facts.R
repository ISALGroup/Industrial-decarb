...
emissions_data_nat <- 
  ...
  summarize(
    ...
    lifetime_elec_mt = mean(lifetime_elec_mt, na.rm = TRUE),
  ...
  summarize(
    ...
    lifetime_elec_mmt = sum(lifetime_elec_mt, na.rm = TRUE)/ 1e6, 
  ...
  mutate(
    emissions_label = case_when(
      ...
      emissions_type == 'lifetime_elec_mmt' ~ names(fig_emissions_colors)[5], 
...
top_payback_data_nopol <- 
  ...
  pivot_longer(
    cols = c(payback_years, elec_mmt_25),
    names_to = "metric",
    values_to = "value"
  ) |>
  group_by(statesub) |>
  ...
  mutate(
    ...
    metric = fct_relevel(metric, "payback_years", "elec_mmt_25")
  )
...
top_payback_plot_nopol <- 
  ...
  facet_wrap(
    ...
    labeller = as_labeller(c(
      ...
      elec_mmt_25    = "Total Emissions (MMt CO2e)"
    )), 
...
delta_npv_xy_state_data <- 
  ...
  summarize(
    ...
    elec_mt_25 = mean(elec_mt_25, na.rm = TRUE)
  ) |>
  ungroup() |>
  mutate(
    npv_delta_pct = 100 * (npv - npv_ng) / npv_ng
  ...
  summarize(
    ...
    elec_mt_25 = sum(elec_mt_25, na.rm = TRUE) / 1000000
  ) |>
  ungroup() |>
  mutate(
    statesub = paste(state, industry_clean, sep = " "),
  ...
  select(state, industry_clean, sector, policy_label, npv_delta_pct, elec_mt_25) |>
  filter(!is.na(npv_delta_pct), abs(npv_delta_pct) < 1000) |>
  arrange(desc(elec_mt_25))
...
eim_data_nat <- 
  ...
  summarize(
    ...
    lifetime_elec_mmt = sum(lifetime_low_med_effective, na.rm = T) / 1e6, 
    lifetime_chp_mmt = sum(lifetime_chp_mt, na.rm = TRUE)/ 1e6, 
    lifetime_biogenic_mmt = sum(lifetime_biogenic_mt, na.rm = TRUE)/ 1e6, 
    lifetime_no_direct_mmt = sum(lifetime_no_direct_mt, na.rm = TRUE)/ 1e6, 
    lifetime_hightemp_mmt = sum(lifetime_hightemp_mt, na.rm = TRUE)/ 1e6
